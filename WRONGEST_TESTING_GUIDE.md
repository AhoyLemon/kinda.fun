...
- [ ] Click "Create a New Room" button on title screen
- [ ] Verify room code is generated (5 characters, all uppercase)
- [ ] Verify URL updates to include `?room={ROOMCODE}`
- [ ] Check browser console for successful Firestore document creation
- [ ] Verify room document exists in Firestore: `rooms/{roomCode}`

**Test: Join an Existing Room**
- [ ] In a second browser window, enter the room code
- [ ] Click "Join this room" button
- [ ] Verify player joins the room successfully
- [ ] Verify URL updates with room code
- [ ] Check that player list updates in both windows
