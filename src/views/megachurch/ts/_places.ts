import { Tags, ReligionNames, Place, PlaceNames } from "./_types";

export const places: Place[] = [
  {
    id: 0,
    name: "Starting Location",
    description:
      "A busy street corner in an average American town where all walks of life pass by. You've set up your makeshift pulpit on the sidewalk, hoping to catch the attention of commuters, shoppers, and anyone willing to listen. The crowd is diverse—some hurried, some curious, most indifferent—but a few coins in your hat at the end of the day might just cover your basic needs. This is where every great preacher starts: with nothing but a voice and a dream.",
    likedTags: ["community", "sincerity", "compassion", "simplicity"],
    dislikedTags: ["outrageousness", "bizarre concepts", "violence"],
    totalPopulation: 100000,
    avgNetWorth: 55000,
    religions: [
      { id: 1, name: "Southern Baptist", weight: 18 },
      { id: 11, name: "Secular Humanists", weight: 18 },
      { id: 2, name: "Catholicism", weight: 15 },
      { id: 8, name: "Evangelical Christianity", weight: 12 },
      { id: 27, name: "Agnostic Atheists", weight: 12 },
      { id: 9, name: "Mormonism", weight: 8 },
      { id: 10, name: "Jehovah's Witnesses", weight: 8 },
      { id: 26, name: "Buddhism", weight: 6 },
      { id: 13, name: "New-Age Spirituality", weight: 6 },
      { id: 31, name: "The Church of Having More Things", weight: 6 },
      { id: 21, name: "The Starseed Pilgrims", weight: 5 },
      { id: 28, name: "Seventh-day Adventism", weight: 5 },
      { id: 17, name: "The Gaia Collective", weight: 4 },
      { id: 4, name: "The Church of Eros", weight: 4 },
      { id: 24, name: "The Church of Satan", weight: 4 },
      { id: 14, name: "The Church of the High Priest", weight: 4 },
      { id: 16, name: "The Digital Ascension", weight: 3 },
      { id: 22, name: "The Order of the Divine Algorithm", weight: 3 },
      { id: 30, name: "Confucianism", weight: 3 },
      { id: 25, name: "Theistic Satanism", weight: 3 },
      { id: 18, name: "The Order of the Alpha Male", weight: 3 },
      { id: 15, name: "2 Fast 2 Faithful", weight: 2 },
      { id: 23, name: "Temple of the Eternal Sun", weight: 2 },
      { id: 5, name: "Zardoz", weight: 2 },
      { id: 20, name: "The Sovereign Guild of Untethered Minds", weight: 2 },
      { id: 19, name: "The Queens' Dominion", weight: 2 },
    ],
  },
  {
    id: 1,
    name: "A-Hyuk Hyuk, Alabama",
    description:
      "A-Hyuk Hyuk, Alabama is a poor, fiercely conservative outpost where country music blares from every battered pickup truck and every shirt, hat, and bumper sticker is a declaration of faith and politics. Church is the social center, the diner is the confessional, and every meal comes with a side of grease and gospel. Outsiders are met with suspicion, but neighbors are family—especially if you share their love of fried food, flag-waving, and a preacher who can outshoot the sheriff. If you can sell salvation with a twang and a coupon for free fries, you'll fit right in.",
    likedTags: [
      "community",
      "loyalty",
      "guns",
      "family values",
      "patriotism",
      "tradition",
      "nationalism",
    ],
    dislikedTags: [
      "feminism",
      "nihilism",
      "modernity",
      "liberal politics",
      "skepticism",
      "secularism",
    ],
    totalPopulation: 90000,
    avgNetWorth: 70000,
    religions: [
      { id: 1, name: "Southern Baptist", weight: 63 },
      { id: 18, name: "The Order of the Alpha Male", weight: 44 },
      { id: 8, name: "Evangelical Christianity", weight: 42 },
      { id: 15, name: "2 Fast 2 Faithful", weight: 18 },
      { id: 31, name: "The Church of Having More Things", weight: 12 },
      { id: 5, name: "Zardoz", weight: 12 },
      { id: 14, name: "The Church of the High Priest", weight: 11 },
      { id: 28, name: "Seventh-day Adventism", weight: 7 },
      { id: 10, name: "Jehovah's Witnesses", weight: 6 },
      { id: 27, name: "Agnostic Atheists", weight: 3 },
      { id: 24, name: "The Church of Satan", weight: 2 },
    ],
  },
  {
    id: 2,
    name: "Yankee Grit, New Hampshire",
    description:
      "Yankee Grit, New Hampshire is a stubborn stronghold of flannel, granite, and suspiciously overpriced maple syrup. Independence is the local religion, but the right preacher can convince anyone to buy a $99 prayer blanket or sign up for salvation by mail. Town meetings are battlegrounds for conspiracy theories and passive-aggressive bake sales, and every neighbor is either a rugged individualist or a secret member of a faith-based pyramid scheme. If you can sell freedom with a side of spiritual snake oil, these folks will buy it twice and argue about it for generations.",
    likedTags: [
      "individualism",
      "tradition",
      "skepticism",
      "nationalism",
      "freedom",
      "self-sufficiency",
    ],
    dislikedTags: [
      "authority",
      "liberal politics",
      "modernity",
      "other religions",
      "progress",
    ],
    totalPopulation: 65000,
    avgNetWorth: 90000,
    religions: [
      { id: 20, name: "The Sovereign Guild of Untethered Minds", weight: 44 },
      { id: 11, name: "Secular Humanists", weight: 27 },
      { id: 2, name: "Catholicism", weight: 25 },
      { id: 27, name: "Agnostic Atheists", weight: 20 },
      { id: 21, name: "The Starseed Pilgrims", weight: 19 },
      { id: 24, name: "The Church of Satan", weight: 18 },
      { id: 8, name: "Evangelical Christianity", weight: 14 },
      { id: 4, name: "The Church of Eros", weight: 12 },
      { id: 17, name: "The Gaia Collective", weight: 10 },
      { id: 31, name: "The Church of Having More Things", weight: 9 },
      { id: 26, name: "Buddhism", weight: 3 },
    ],
  },
  {
    id: 3,
    name: "Venture City, California",
    description:
      "Venture City is a glittering dystopia where tech billionaires are living gods and their every utterance—especially after a $50,000 ayahuasca retreat—is treated as gospel. The city is ruled by algorithmic dogma, and the faithful gather in glass towers to worship at the altar of disruption, hoping for a retweet from their digital messiah. Residents are obsessed with innovation, but only if it comes with a six-figure price tag and a TED Talk. Spirituality is a commodity, enlightenment is a subscription, and the only thing more sacred than venture capital is the latest miracle cleanse. If you can sell salvation as an app, these zealots will download it, rate it five stars, and demand a blockchain for their soul.",
    likedTags: [
      "innovation",
      "technology",
      "wealth",
      "materialism",
      "progress",
      "individualism",
      "greed",
      "power",
      "consumerism",
    ],
    dislikedTags: [
      "tradition",
      "poverty",
      "authority",
      "humility",
      "simplicity",
    ],
    totalPopulation: 850000,
    avgNetWorth: 350000,
    religions: [
      { id: 22, name: "The Order of the Divine Algorithm", weight: 62 },
      { id: 16, name: "The Digital Ascension", weight: 45 },
      { id: 21, name: "The Starseed Pilgrims", weight: 43 },
      { id: 31, name: "The Church of Having More Things", weight: 40 },
      { id: 13, name: "New-Age Spirituality", weight: 37 },
      { id: 14, name: "The Church of the High Priest", weight: 35 },
      { id: 6, name: "Prosperity Gospel", weight: 25 },
      { id: 27, name: "Agnostic Atheists", weight: 30 },
      { id: 11, name: "Secular Humanists", weight: 22 },
      { id: 26, name: "Buddhism", weight: 20 },
      { id: 19, name: "The Queens' Dominion", weight: 12 },
      { id: 23, name: "Temple of the Eternal Sun", weight: 8 },
    ],
  },
  {
    id: 4,
    name: "Blessed Bayou, Louisiana",
    description:
      "Blessed Bayou, Louisiana is a rural patchwork of winding waterways, weathered porches, and faith as thick as the summer humidity. Here, folks gather for catfish fries, swap miracle stories at the bait shop, and trust the local preacher almost as much as their grandmama. The megachurch is a converted barn, and Sunday service might end with a frog-jumping contest or a raffle for holy mosquito repellent. Tradition runs deep, skepticism runs shallow, and every neighbor is ready to lend a hand—or sell you a blessed gator tooth. If you can promise prosperity with a side of hush puppies, you'll have believers faster than you can say 'amen, y'all.'",
    likedTags: [
      "community",
      "tradition",
      "family values",
      "simplicity",
      "humility",
      "nature",
      "sincerity",
    ],
    dislikedTags: [
      "modernity",
      "skepticism",
      "technology",
      "liberal politics",
      "feminism",
    ],
    totalPopulation: 80000,
    avgNetWorth: 75000,
    religions: [
      { id: 2, name: "Catholicism", weight: 78 },
      { id: 19, name: "The Queens' Dominion", weight: 44 },
      { id: 1, name: "Southern Baptist", weight: 24 },
      { id: 25, name: "Theistic Satanism", weight: 22 },
      { id: 8, name: "Evangelical Christianity", weight: 18 },
      { id: 15, name: "2 Fast 2 Faithful", weight: 15 },
      { id: 24, name: "The Church of Satan", weight: 8 },
      { id: 28, name: "Seventh-day Adventism", weight: 12 },
      { id: 10, name: "Jehovah's Witnesses", weight: 10 },
      { id: 31, name: "The Church of Having More Things", weight: 5 },
    ],
  },
  {
    id: 5,
    name: "Crystal Dunes, Arizona",
    description:
      "Crystal Dunes, Arizona is a sun-baked haven for seekers, mystics, and dreamers. The desert landscape is dotted with crystal shops, meditation retreats, and mysterious stone circles. UFO sightings are a local pastime, and open-mindedness is the norm—especially when it comes to buying blessed moon rocks from a guy named Starbeam. Residents gather for sunrise yoga, swap stories of spiritual awakenings, and cherish the natural beauty that surrounds them, but they're also first in line for any guru promising enlightenment for three easy payments. If you can sell it with a sparkle, they'll believe it with a smile.",
    likedTags: [
      "meditation",
      "nature",
      "crystals",
      "altered states",
      "transcendence",
      "open-mindedness",
      "inner harmony",
      "personal energy",
    ],
    dislikedTags: [
      "skepticism",
      "materialism",
      "violence",
      "authority",
      "technology",
    ],
    totalPopulation: 70000,
    avgNetWorth: 95000,
    religions: [
      { id: 13, name: "New-Age Spirituality", weight: 60 },
      { id: 21, name: "The Starseed Pilgrims", weight: 45 },
      { id: 17, name: "The Gaia Collective", weight: 40 },
      { id: 26, name: "Buddhism", weight: 35 },
      { id: 19, name: "The Queens' Dominion", weight: 30 },
      { id: 16, name: "The Digital Ascension", weight: 30 },
      { id: 27, name: "Agnostic Atheists", weight: 20 },
      { id: 30, name: "Confucianism", weight: 12 },
      { id: 31, name: "The Church of Having More Things", weight: 10 },
    ],
  },
  {
    id: 6,
    name: "Miracle Springs, Florida",
    description:
      "Miracle Springs, Florida is a wild, sun-baked haven where faith and chaos go hand in hand. The locals are fiercely religious, but their devotion is matched only by their unpredictability—expect street preachers wrestling alligators, drive-thru exorcisms, and miracle cures sold out of airboats. Every sunrise is a sign from above, every sunset a chance for a new scandal, and Sunday services are as likely to end in a brawl as a baptism. The community thrives on outrageous stories, impulsive acts, and a belief that the end times are just around the corner. If you can survive the madness, you'll find a congregation ready to follow you into the swamp—or straight into the headlines.",
    likedTags: [
      "chaos",
      "blind faith",
      "bizarre concepts",
      "outrageousness",
      "passion",
      "sensory experience",
      "speed",
    ],
    dislikedTags: [
      "order",
      "skepticism",
      "secularism",
      "rationalism",
      "self-control",
    ],
    totalPopulation: 120000,
    avgNetWorth: 80000,
    religions: [
      { id: 14, name: "The Church of the High Priest", weight: 49 },
      { id: 5, name: "Zardoz", weight: 33 },
      { id: 15, name: "2 Fast 2 Faithful", weight: 30 },
      { id: 4, name: "The Church of Eros", weight: 30 },
      { id: 19, name: "The Queens' Dominion", weight: 25 },
      { id: 18, name: "The Order of the Alpha Male", weight: 24 },
      { id: 16, name: "The Digital Ascension", weight: 20 },
      { id: 28, name: "Seventh-day Adventism", weight: 20 },
      { id: 10, name: "Jehovah's Witnesses", weight: 20 },
      { id: 25, name: "Theistic Satanism", weight: 20 },
      { id: 23, name: "Temple of the Eternal Sun", weight: 20 },
      { id: 31, name: "The Church of Having More Things", weight: 15 },
    ],
  },
  {
    id: 7,
    name: "Landing Strip, Nevada",
    description:
      "Landing Strip, Nevada is a neon-lit playground where the pursuit of wealth and pleasure is the true religion. The locals are obsessed with luck, luxury, and living large—faith is measured in jackpots and spa packages, not sermons. Megachurches double as casinos, and every wedding chapel offers a complimentary spiritual cleanse with your buffet. Residents chase the next big win, the wildest party, and the most extravagant experience, believing that happiness is just one jackpot away. If you can sell prosperity, pleasure, or redemption chips, you'll be a high roller in no time.",
    likedTags: [
      "wealth",
      "pleasure",
      "materialism",
      "greed",
      "money",
      "power",
      "consumerism",
    ],
    dislikedTags: [
      "poverty",
      "humility",
      "self-control",
      "tradition",
      "simplicity",
    ],
    totalPopulation: 600000,
    avgNetWorth: 180000,
    religions: [
      { id: 6, name: "Prosperity Gospel", weight: 56 },
      { id: 4, name: "The Church of Eros", weight: 44 },
      { id: 22, name: "The Order of the Divine Algorithm", weight: 40 },
      { id: 31, name: "The Church of Having More Things", weight: 35 },
      { id: 23, name: "Temple of the Eternal Sun", weight: 32 },
      { id: 24, name: "The Church of Satan", weight: 30 },
      { id: 27, name: "Agnostic Atheists", weight: 25 },
      { id: 2, name: "Catholicism", weight: 20 },
      { id: 9, name: "Mormonism", weight: 18 },
      { id: 8, name: "Evangelical Christianity", weight: 15 },
      { id: 1, name: "Southern Baptist", weight: 8 },
    ],
  },
  {
    id: 8,
    name: "Megamall, Minnesota",
    description:
      "Megamall, Minnesota is a sprawling, ultra-modern monument to consumerism, where every store is a cathedral to shopping and every escalator is a pilgrimage. The population is predominantly white and proudly liberal, with progressive values on display in every food court and boutique. Religion takes a back seat to retail therapy, and the local megachurch is more of a community center than a spiritual hub—hosting yoga classes, book clubs, and pop-up shops. Residents are obsessed with the latest trends, tech gadgets, and artisanal snacks, and spirituality is often replaced by brand loyalty and the pursuit of the perfect sale. If you can preach the gospel of consumption, you'll find a congregation eager for the next big thing.",
    likedTags: [
      "consumerism",
      "modernity",
      "progress",
      "rules",
      "order",
      "community",
      "compassion",
      "social justice",
      "liberal politics",
      "speed",
      "positive affirmations",
    ],
    dislikedTags: [
      "poverty",
      "dogma",
      "outrageousness",
      "rebellion",
      "violence",
      "nationalism",
      "apathy",
    ],
    totalPopulation: 350000,
    avgNetWorth: 120000,
    religions: [
      { id: 31, name: "The Church of Having More Things", weight: 60 },
      { id: 27, name: "Agnostic Atheists", weight: 24 },
      { id: 2, name: "Catholicism", weight: 29 },
      { id: 19, name: "The Queens' Dominion", weight: 27 },
      { id: 4, name: "The Church of Eros", weight: 23 },
      { id: 26, name: "Buddhism", weight: 14 },
      { id: 13, name: "New-Age Spirituality", weight: 12 },
      { id: 8, name: "Evangelical Christianity", weight: 10 },
      { id: 9, name: "Mormonism", weight: 7 },
      { id: 15, name: "2 Fast 2 Faithful", weight: 6 },
      { id: 24, name: "The Church of Satan", weight: 6 },
    ],
  },
  {
    id: 9,
    name: "Cascadia Moss, Washington",
    description:
      "Cascadia Moss, Washington is a vibrant, tech-savvy refuge in the Pacific Northwest, where brewmasters, startup founders, and digital prophets mingle in bustling coffee shops and rooftop gardens. The city pulses with innovation, eco-conscious living, and a laid-back hippie spirit—think kombucha on tap, artisan donuts, and meditation apps. The local megachurch is a glass-walled hub for TED-style sermons, mindfulness workshops, and networking mixers. Here, spirituality is as likely to be found in a blockchain seminar as in a mossy park, and the only thing more abundant than rain is the Wi-Fi. If you can sell enlightenment with a craft IPA, you'll have a loyal following in no time.",
    likedTags: [
      "technology",
      "sustainability",
      "innovation",
      "nature",
      "meditation",
      "inner harmony",
      "free thought",
      "progress",
    ],
    dislikedTags: ["tradition", "materialism", "nationalism", "dogma"],
    totalPopulation: 150000,
    avgNetWorth: 130000,
    religions: [
      { id: 11, name: "Secular Humanists", weight: 45 },
      { id: 26, name: "Buddhism", weight: 40 },
      { id: 13, name: "New-Age Spirituality", weight: 35 },
      { id: 16, name: "The Digital Ascension", weight: 32 },
      { id: 27, name: "Agnostic Atheists", weight: 30 },
      { id: 22, name: "The Order of the Divine Algorithm", weight: 28 },
      { id: 17, name: "The Gaia Collective", weight: 25 },
      { id: 8, name: "Evangelical Christianity", weight: 20 },
      { id: 19, name: "The Queens' Dominion", weight: 18 },
      { id: 2, name: "Catholicism", weight: 15 },
      { id: 31, name: "The Church of Having More Things", weight: 8 },
    ],
  },
  {
    id: 10,
    name: "Rapture, Texas",
    description:
      "Rapture, Texas is a dusty, struggling town where hope is scarce and tempers run hot. The community is fiercely devoted to faith, clinging to apocalyptic sermons and waiting for the end times to bring justice and salvation. The local megachurch is a fortress of prophecy, and Sunday services are filled with fiery warnings and calls for repentance. Residents are poor, angry, and deeply suspicious of outsiders, but united in their belief that the world is about to change—hopefully in their favor. If you can preach doom and deliverance, you'll find a congregation desperate for answers and ready for the rapture.",
    likedTags: [
      "prophecy",
      "blind faith",
      "suffering",
      "hardship",
      "justice",
      "strength",
      "transcendence",
    ],
    dislikedTags: [
      "wealth",
      "secularism",
      "modernity",
      "progress",
      "liberal politics",
    ],
    totalPopulation: 70000,
    avgNetWorth: 75000,
    religions: [
      { id: 18, name: "The Order of the Alpha Male", weight: 60 },
      { id: 8, name: "Evangelical Christianity", weight: 55 },
      { id: 5, name: "Zardoz", weight: 50 },
      { id: 1, name: "Southern Baptist", weight: 45 },
      { id: 10, name: "Jehovah's Witnesses", weight: 40 },
      { id: 28, name: "Seventh-day Adventism", weight: 35 },
      { id: 15, name: "2 Fast 2 Faithful", weight: 25 },
      { id: 6, name: "Prosperity Gospel", weight: 20 },
      { id: 25, name: "Theistic Satanism", weight: 20 },
      { id: 4, name: "The Church of Eros", weight: 15 },
      { id: 20, name: "The Sovereign Guild of Untethered Minds", weight: 6 },
    ],
  },
];
